					...
					<!-- ===== Header ===== -->
						<div class="app-header">
							...
							<div class="app-header-left">
								...
								<p class="app-name"> <? print($website_info['Title']);?></p>
							</div>
							<!-- Search -->
								...
								<?
									...
									foreach($items as $item) {
										if($userperm[$item['Permission']] == 1) {
											...
											if($item['Counter ID'] > 0 && ($count =	mysqli_fetch_row(DB_Query(mysqli_fetch_row(DB_Query(sprintf("SELECT `SQL String` FROM `Item counters` WHERE `Active`=1 AND `ID`=%s", $item['Counter ID'])))[0]))[0]) > 0) {
												print(sprintf('
