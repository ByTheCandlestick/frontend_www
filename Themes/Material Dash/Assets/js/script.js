$( document ).ready(() => {
	...
	// -----========== Nestled functions ==========----- //
		...
		website = {
			...
			/** @final */
			page: {
				styles: $("div[name=styles]").children().find("input[type=checkbox]:checked").each((index, element) => { page.styles.push($(element).val()); }),
				scripts: $("div[name=scripts]").children().find("input[type=checkbox]:checked").each((index, element) => { page.scripts.push($(element).val()); }),
				data: {
					'api_key': api_key,
					'style': styles.join(","),
					'script': scripts.join(","),
