	...
	<?
		if(isset($secext) && $secext == 'products') {
			//
				...
				$start = ($page - 1) * 16;
				$prd_viewed = $page * 16;
				$count = mysqli_fetch_row(DB_Query("SELECT COUNT(*) FROM `Product info` WHERE `made_by_ID`=1 AND `Active`=1 AND `Discontinued`=0"))[0];
			//
			$query = DB_Query("SELECT * FROM `Product info` WHERE `made_by_ID`=1 AND `Active`=1 AND `Discontinued`=0 LIMIT $start, 16");
			...
			$query = DB_Query("SELECT * FROM `Product info` WHERE `made_by_ID`='$part_ID' AND `Active`=1 AND `Discontinued`=0");
			if(mysqli_num_rows($query) > 0){
				while($row = mysqli_fetch_array($query)) {
					$prod_price = $row['RetailPrice'];
					$prod_slug = $row['Slug'];
