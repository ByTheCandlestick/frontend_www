<?
	if(mysqli_num_rows($query = DB_Query(sprintf("SELECT * FROM `User accounts` WHERE `ID`=%s", QS))) > 0) {
		$user = mysqli_fetch_assoc($query);
		$userperm = mysqli_fetch_assoc(DB_Query(sprintf("SELECT * FROM `User permissions` WHERE `UID`=%s", QS)));
?>
	<section>
		<!-- Section Header -->
		<div class="row">
			...
			<div class="col-12 col-md-6 text-md-end">
				<div class="row">
					<div class="col-12 d-block d-md-flex justify-content-end align-items-center p-0">
						<a href="javascript:user.savePerm(<?=(QS)?>);" class="btn btn-outline-primary m-1">
