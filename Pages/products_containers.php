...
?><?
    ...
    $q = DB_Query($prnt = sprintf("SELECT * FROM `Product containers` ORDER BY `ID` ASC LIMIT %s OFFSET %s", $config['Maximum list size'] OFFSET $offset));
				...
				<?
					if(count($containers) > 0) {
						foreach($containers as $x) {
							$editable = ($userperm['adm_access-products-containers-edit']==1)?'<a href="/Products/Container/'.$x['ID'].'">'.$x['Name'].'</a>':$x['Name'];
							print('
								<tr>
