						...
						while ($row = mysqli_fetch_array($query)) {
							print('
								<tr>
									...
									<td><a href="javascript:misc.copyToClipboard(\''.$row['ItemRef'].'\');alert.simple(\'Copied. Please search for this item in the new tab\', \'info\');setTimeout(function(){misc.openInNewTab(\''.$suppliers[$row['Supplier']]['Website'].'\');},5000);">'.$row['ItemRef'].'</a></td>
									<td>'.$row['Size (cl)'].'</td>
									<td>'.$row['Price (ea)'].'</td>
									<td>'.$row['Active'].'</td>
									<td>
