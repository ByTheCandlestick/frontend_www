...
?><?
    ...
    $q = DB_Query(sprintf("SELECT * FROM `Transactions` ORDER BY `Modified` DESC LIMIT %s OFFSET %s", $config['Maximum list size'], $offset));
...
<section>
	...
	<div class="row overflow-scroll">
					...
					if(count($transactions) > 0) {
						foreach($transactions as $x) {
							...
							print('
								<tr>
									<th scope="row">
										<a href="/Transactions/'.$x['Transaction ID'].'/">'.$x['Transaction ID'].'</a>
									</th>
									<td>'.$x['Type'].'</td>
									<td>'.$x['Status'].'</td>
		...
		<?
			...
			(($offset + $config['Maximum list size']) < $total_transactions)? $next_status = '': $next_status = ' disabled';
