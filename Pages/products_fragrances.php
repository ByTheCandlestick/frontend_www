...
?><?
    ...
    $q = DB_Query($prnt = sprintf("SELECT * FROM `Product fragrances` ORDER BY `ID` ASC LIMIT %s OFFSET %s", $config['Maximum list size'] OFFSET $offset));
...
<section>
	...
	<div class="row">
		<div class="col-12 col-md-6">
			<h1>Fragrances</h1>
			<p>Displaying: <?=($offset > 1)? ($offset + 1).'-'.($offset + count($fragrances)): count($fragrances);?>/<?=$total_fragrances?> Rows</p>
		</div>
		<div class="col-12 col-md-6 text-md-end">
			<div class="row">
				<div class="col-12 col-lg-6 d-block d-md-flex justify-content-end align-items-center p-0">
					<a href="/Products/Fragrance/New/" class="btn btn-outline-primary m-1">
			...
			<tbody>
				<?
					if(count($fragrances) > 0) {
						foreach($fragrances as $x) {
							$editable = ($userperm['adm_access-products-fragrances-edit']==1)?'<a href="/Products/Fragrance/'.$x['ID'].'">'.$x['Name'].'</a>':$x['Name'];
							print('
								<tr>
...
<script>
	$(document).ready(function(){
		$(".tableFilter").on("keyup", function() {
			...
			$(".fragranceTable tbody tr").filter(function() {
