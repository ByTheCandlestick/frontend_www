	...
	class StripeModel extends BaseModel {
		 ...
		 */
			public function createCustomer($stripe, $fname, $lname, $phone, $email, $uid, $address1, $address2, $town, $county, $postcode, $token) {
				$this->uploadAudit(__FUNCTION__, (new ReflectionFunction(__FUNCTION__))->getParameters(), "Created a new customer", "Orders", $uid);
		 ...
		 */
			public function createCharge($stripe, $customer, $pricePennies, $currency, $items, $uid) {
				$this->uploadAudit(__FUNCTION__, (new ReflectionFunction(__FUNCTION__))->getParameters(), "Created a new charge", "Orders", $uid);
		 ...
		 */
			public function uploadSalesOrder($invoice_number, $uid, $name, $email, $phone, $items, $notes, $shipping, $address_id, $price, $paidAmount, $fees, $currency, $status, $txn, $chg, $paymentStatus, $network, $last4, $exp_M, $exp_Y) {
				...
				$this->uploadAudit(__FUNCTION__, (new ReflectionFunction(__FUNCTION__))->getParameters(), "Uploaded Sales Order", "Orders", $uid);
		 ...
		 */
			public function uploadAddress($uid, $number, $address1, $address2, $town, $county, $country, $postcode) {
				$this->uploadAudit(__FUNCTION__, (new ReflectionFunction(__FUNCTION__))->getParameters(), "Upload customer address", "Orders", $uid);
		 ...
		 */
			public function uploadRefund($id, $amount, $currency, $bal_txn, $ch_id, $status) {
				$val = $amount / 100;
				$this->uploadAudit(__FUNCTION__, (new ReflectionFunction(__FUNCTION__))->getParameters(), "Created a new Refund", "Refunds", $uid);
