	...
	class StripeModel extends BaseModel {
		 ...
		 */
			public function uploadSalesOrder($invoice_number, $uid, $name, $email, $phone, $items, $notes, $shipping, $address_id, $price, $paidAmount, $fees, $currency, $status, $txn, $chg, $paymentStatus, $network, $last4, $exp_M, $exp_Y) {
				...
				$this->Execute(sprintf("INSERT INTO `Transactions` (`Transaction ID`, `Type`, `Status`, `Invoice ID`, `Charge ID`, `Subtotal`, `Processing Fees`, `Tax`, `Deposit`, `Currency`, `Notes`, `UID`, `Name`, `Email`, `Phone`, `Items`, `Ship to`, `Shipping by`, `Billing address`, `Estimated del date`, `Card network`, `Last 4`, `Expires month`, `Expires year`, `Modified`, `Created`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', now(), now())", $txn, 'Order', $paymentStatus, $invoice_number, $chg, $price, $fees, (round(($paidAmount - $fees)*0.2, 2)), $paidAmount, $currency, $notes, $uid, $name, $email, $phone, $items, '', $shipping, $address_id, date("d/m/Y", time()+3600*24*10), $network, $last4, $exp_M, $exp_Y), 1);
			}
		/** uploadAddress
		 * 
		 * 
