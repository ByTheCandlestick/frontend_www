<?
	class PageModel extends BaseModel {
		 ...
		 * @result boolean
		 */
			public function CreatePage(array $info, string $uid) {
				$vars = array(array('info', $info), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Created a new page", "Pages", $uid);
		 ...
		 * @result boolean
		 */
			public function UpdatePage(string $sid, array $info, string $uid) {
				$vars = array(array('sid', $sid), array('info', $info), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Updated a page", "Pages", $uid);
				$menu_url = ($info['subpage_url']!='')? '/'.ucfirst($info['page_url']).'/'.ucfirst($info['subpage_url']).'/': '/'.ucfirst($info['page_url']).'/';
		 ...
		 * @result boolean
		 */
			public function DeletePage(string $pid, string $uid) {
				$vars = array(array('pid', $pid), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Deleted a page", "Pages", $uid);
		 ...
		 */
			public function UpdateLayout(string $pid, array $info, string $uid) {
				$vars = array(array('pid', $pid), array('info', $info), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Set page layout", "Pages", $uid);
