	...
	class ProductModel extends BaseModel {
		 ...
		 */
			public function updateProduct(string $sku, array $info, string $uid) {
				$vars = array(array('sku', $sku), array('info', $info), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Updated a Product", "Products", $uid);
		 ...
		 */
			public function createSKU(string $uid) {
				if($this->Execute($q="SELECT `SKU` FROM `Product` ORDER BY SKU DESC LIMIT 1", 5)>0) {
					$sku = intval($this->Execute($q, 3)['SKU']) + 1;
				} else {
					$sku = 10001;
				}
				$vars = array(array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Created a new SKU: ".$sku, "Products", $uid);
				return $sku;
			}
		/** createUPC
		 * 
		 ...
		 */
			public function createUPC(string $productCode, array $info, string $uid) {
				$vars = array(array('productCode', $productCode), array('info', $info), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Created a new UPC", "Products", $uid);
				$UPC  = '7';
		 ...
		 */
			public function createProduct(string $sku, string $upc, array $info, string $uid) {
				$vars = array(array('sku', $sku), array('upc', $upc), array('info', $info));
				$this->uploadAudit(__FUNCTION__, $vars, "Created a new Product", "Products", $uid);
		 ...
		 */
			public function deleteProduct(string $sku, string $uid) {
				$vars = array(array('sku', $sku), array('uid', $uid));
				$this->uploadAudit(__FUNCTION__, $vars, "Celeted a product", "Products", $uid);
