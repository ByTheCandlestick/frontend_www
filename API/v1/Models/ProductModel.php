	...
	class ProductModel extends BaseModel {
		 ...
		 *	@return string
			*/
			public function createSKU() {
				if($this->Execute("SELECT `SKU` FROM `products` ORDER BY SKU DESC LIMIT 1", 5)>0) {
					return print_r(intval($this->Execute("SELECT `SKU` FROM `products` ORDER BY SKU DESC LIMIT 1", 4)['SKU']));
				} else {
					return 10001;
				}
			}
		 ...
		 */
			public function createUPC(string $productCode) {
				$UPC  = 7;
				$UPC .= $this->Execute(sprintf("SELECT `Reference` FROM `partners` WHERE `ID`='%s'", $info['made_by']), 4)['Reference'];
				$UPC .= $productCode;
				$i=1;
