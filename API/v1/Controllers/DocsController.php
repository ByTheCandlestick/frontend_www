		...
		function InvoiceTable($header, $w, $data, $posx, $posy, $drawCol = array(0, 0, 0), $textCol = array(0, 0, 0), $fontSize) {
			...
			foreach($data as $row) {
				...
				if($row[0] == "totalRow") {
					...
					$this->Cell($w[1], 6, number_format(floatval($row[1]), 2), 'B', 0, 'C');
						...
						try{
							...
							// Content
								// Invoice ID
									...
									$pdf->Cell($pdf->GetStringWidth($address['number_name'].' '.$address['line_1']), $pdf->GetStringHeight($pdf->fs_h5), $address['number_name'].' '.$address['line_1'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($address['line_2']), $pdf->GetStringHeight($pdf->fs_h5), $address['line_2'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($address['town']), $pdf->GetStringHeight($pdf->fs_h5), $address['town'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($address['county']), $pdf->GetStringHeight($pdf->fs_h5), $address['county'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($address['country']), $pdf->GetStringHeight($pdf->fs_h5), $address['country'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($address['postcode']), $pdf->GetStringHeight($pdf->fs_h5), $address['postcode'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($invoice['Email']), $pdf->GetStringHeight($pdf->fs_h5), $invoice['Email'], $pdf->dev_outline, 1, "L");
									$pdf->Cell($pdf->GetStringWidth($invoice['Phone']), $pdf->GetStringHeight($pdf->fs_h5), $invoice['Phone'], $pdf->dev_outline, 1, "L");
								// Invoice Date / ID
									$pdf->SetFont('Raleway', 'B', $pdf->fs_p);
									...
									$pdf->Cell($pdf->GetStringWidth($invoice['Created']), $pdf->GetStringHeight($pdf->fs_h5), $invoice['Created'], $pdf->dev_outline, 1, "R");
								// invoice Table
